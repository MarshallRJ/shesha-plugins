...
**So: set the app theme once for chrome, then stop.** Do not spend a session tuning theme tokens expecting the page to transform — it won't, and that time is the single most reliably wasted hour in this pipeline. High fidelity comes from composing pre-styled blocks with literal values: `../assets/house-style.json` and `shesha-form-edit/assets/blocks/`.
