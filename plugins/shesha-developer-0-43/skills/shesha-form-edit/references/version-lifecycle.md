...
On Shesha **0.43.x** a form configuration is a **versioned ConfigurationItem**, not a mutable blob.
...
component/reference doc that mentions pushing/publishing **links here** rather than re-deriving the
algorithm. Everything below was **verified against a live 0.43 backend** (`localhost:21021`,
2026-08-19); the ⚠ notes flag where the running build differs from the naive contract.
...
`versionStatus` is an integer. The 0.43 values (all observed in the wild — a live backend showed a
`{Draft:3, Live:689, Retired:1}` distribution across 693 forms):
...
| 4 | Cancelled | an abandoned Draft/Ready | **NEVER** — resolve to latest non-terminal first |
...
version to Live **auto-retires** the version that was previously Live (it moves to Retired(5)). A
Draft or Ready can also be abandoned → Cancelled(4) (see [Abandoning a version](#abandoning-a-version-cancel)).

---

## Endpoints

All on `$BASE_URL/api/services/Shesha/FormConfiguration/`. ⚠ **Verbs matter** — this build returns
`405 Method Not Allowed` on the wrong verb and `404` when a required id is missing from the body.

| endpoint | verb | body / query | purpose |
|---|---|---|---|
| `GetByName` | GET | `module`, `name`, optional `version` | Resolves the **latest/Live** version by default; pass `version` for a specific `versionNo`. Includes `versionNo`, `versionStatus`, `markup`. |
| `Get` | GET | `id` | ABP envelope for one specific version id — carries `versionNo`, `versionStatus`. |
| `GetJson` | GET | `id` | Raw markup for one version id (file download; the body **is** the form JSON). |
...
| `CreateNewVersion` | POST | `{ "id": "<current version id>" }` | Clones the given version to a **new Draft** (`versionNo`+1, same `Origin`). Returns the **new version's id**. ⚠ `id` is mandatory — an empty `{}` body **404s** (model-binding, not a missing route). |
| `UpdateStatus` | PUT | `{ "filter": "<stringified JSON-logic>", "status": <int> }` | Advances a version's status. ⚠ `filter` is **mandatory** (empty body → `400 "Filter is mandatory"`). Validates Draft→Ready→Live; auto-retires the prior Live. Also accepts `status:4` to Cancel. |
| `UpdateMarkup` | PUT | `{ "id": "<version id>", "markup": "<stringified JSON>" }` | Writes markup to a **specific version id** (see [api.md §5](api.md)). ⚠ PUT only (POST → 405). |
| `ImportJson` | POST | multipart `ItemId` + `file` | Writes markup to a specific version id (see [api.md §6](api.md)). |
| `CancelVersion` | POST | `{ "id": "<ready id>" }` | Abandons a version → Cancelled(4). ⚠ **allowed ONLY on a `Ready(2)` version** on this build — see below. |

> **`UpdateMarkup`/`ImportJson` write to whatever version id you give them.** On 0.43 you must give
> them a **Draft** (or Ready) id — never a Live id. Getting the id wrong is how a Live version gets
> clobbered with no history.

⚠ **`GetAll` projection quirk:** `module.name` comes back `null` in the `GetAll` list projection even
though the module is set — resolve the module via `GetByName`/`Get` when you need it, and don't treat a
null `module.name` as a broken form. `isLast` is **filterable** but is **not serialized back** on the
returned items, so don't assert on `item.isLast` — trust the filter.

⚠ **Swagger is unreliable on 0.43** — `/swagger/v1/swagger.json` may 500. Probe endpoints directly
rather than enumerating from swagger.
...
never `$FORM_ID`.** ⚠ Passing no `id` (or an empty body) returns **404**, which looks like a missing
route but is a missing-argument error — always send a real id.
...
The `filter` value is a **stringified** JSON-logic expression (the same dialect `GetAll` uses) and is
**mandatory** — an empty body returns `400 "Filter is mandatory"`. Filtering by the version `id` is the
most precise; you can also filter by `origin.id` + `versionNo`. Do the two hops as separate calls — some
builds reject a Draft→Live jump.

### Abandoning a version (cancel)

⚠ **`CancelVersion` on this 0.43 build is allowed ONLY on a `Ready(2)` version** — calling it on a
Draft(1) returns `400 "This operation is allowed only for items with 'Ready' status"`. So the documented
"CancelVersion abandons a Draft" is **wrong for this build**. Two reliable paths:

```bash
# A Draft(1) you created but decided not to publish → Cancelled(4) via UpdateStatus (VERIFIED working):
curl -s -X PUT "$BASE_URL/api/services/Shesha/FormConfiguration/UpdateStatus" \
  -H "Authorization: Bearer $ACCESS_TOKEN" -H "Content-Type: application/json" \
  -d "{\"filter\":\"{\\\"and\\\":[{\\\"==\\\":[{\\\"var\\\":\\\"id\\\"},\\\"$NEW_ID\\\"]}]}\",\"status\":4}"

# A Ready(2) version → Cancelled(4) via CancelVersion:
curl -s -X POST "$BASE_URL/api/services/Shesha/FormConfiguration/CancelVersion" \
  -H "Authorization: Bearer $ACCESS_TOKEN" -H "Content-Type: application/json" \
  -d "{\"id\":\"$READY_ID\"}"
```

Use these only in failure recovery, to clean up a version you created but decided not to publish. Never
cancel a Live version. **Prefer `UpdateStatus status:4` for Drafts** — it works regardless of whether the
Draft has been advanced to Ready.
...
latest item is a Draft(1) or Ready(2), **reuse it** — do NOT `CreateNewVersion` again. If it is Live(3),
you'll `CreateNewVersion` from it. If it is Retired(5)/Cancelled(4) with no newer version, resolve to the
latest non-terminal version before touching anything. (⚠ `isLast` is filterable but not echoed back on
items — trust the filter, not `item.isLast`.)

Node helper to resolve the full lifecycle context in one shot:

```js
// RESOLVE: given module+name, return { id, versionNo, versionStatus, originId }
...
- **Brand-new form (no version exists — fresh `Create`)**
  ...
  4. Clear the frontend cache (see [Version-aware verification](#version-aware-verification)).
...
- **Draft(1) or Ready(2) in flight**
  ...
  3. Publish (`UpdateStatus` up to 3) when gates pass; clear the cache. (A Ready only needs 2→3.)
...
5. **Abandon a Draft with `UpdateStatus status:4`, not `CancelVersion`** (which requires Ready on this
   build). Every edit session ends with its Draft either **Live** or **Cancelled** — never dangling.
...
- **`UpdateMarkup` failed (non-200)** — the Draft exists but is unedited (or partially). Re-fetch and
  ...
  keeps failing, surface the raw error and stop; optionally cancel the empty Draft (`UpdateStatus status:4`).
...
- **`UpdateStatus` (publish) failed after a good `UpdateMarkup`** — the Draft exists, correctly edited,
  ...
  - **abort**, and cancel the orphaned Draft (`UpdateStatus status:4`) so it doesn't linger as a stale
    `isLast`.

- **Interrupted mid-session / unsure of state** — run the in-flight lookup (`GetAll` + `isLast==true`).
  If the latest item is a Draft you created, resume from it (edit or publish); never start a new
  version alongside it.

**Golden rule:** exactly one Draft per edit session, and it ends either **Live** (published) or
**Cancelled** (explicitly abandoned via `UpdateStatus status:4`) — never left dangling as an unpublished `isLast`.
...
JS doesn't repopulate mid-clear. Recipe: [verification.md](verification.md).
