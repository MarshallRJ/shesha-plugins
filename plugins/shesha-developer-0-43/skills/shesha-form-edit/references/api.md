...
> **0.43: `UpdateMarkup` writes to whatever version id you give it — give it a DRAFT id, never a Live id.**
> On 0.43 a form is a versioned ConfigurationItem: resolve the version, clone a Live one to a Draft
> (`CreateNewVersion`, §5.1), `UpdateMarkup` the **Draft**, then publish (`UpdateStatus`, §5.1). The full
> algorithm, branch table and invariants are in [version-lifecycle.md](version-lifecycle.md).
...
## 5.1 Version lifecycle — CreateNewVersion + UpdateStatus (0.43)

The 0.43 push is a lifecycle, not a single PUT. Canonical reference (branch table, in-flight lookup,
failure recovery, verified gotchas): [version-lifecycle.md](version-lifecycle.md). The two write calls:

**CreateNewVersion** — clone a Live version to a fresh Draft. `id` is mandatory (an empty body 404s):
...
**UpdateStatus** — publish the Draft `$NEW_ID` in two hops. `filter` is a **mandatory stringified
JSON-logic** expression (empty body → `400 "Filter is mandatory"`):

```bash
publish() { curl -s -X PUT "$BASE_URL/api/services/Shesha/FormConfiguration/UpdateStatus" \
  -H "Authorization: Bearer $ACCESS_TOKEN" -H "Content-Type: application/json" \
  -d "{\"filter\":\"{\\\"and\\\":[{\\\"==\\\":[{\\\"var\\\":\\\"id\\\"},\\\"$1\\\"]}]}\",\"status\":$2}"; }
publish "$NEW_ID" 2   # Draft(1) → Ready(2)
publish "$NEW_ID" 3   # Ready(2) → Live(3)   (auto-retires the prior Live)
```

To **abandon** a Draft use `publish "$NEW_ID" 4` (→ Cancelled). ⚠ `CancelVersion` is allowed only on a
`Ready(2)` version on this build, so prefer `UpdateStatus status:4` for Drafts.

## 6. Push edited markup — ImportJson (multipart upload)

`POST /api/services/Shesha/FormConfiguration/ImportJson` with `multipart/form-data`. Use this when you specifically need to mimic the designer's "upload JSON" button. **0.43: `ItemId` must be a Draft version id (from §5.1 `CreateNewVersion`), never a Live id** — then publish via §5.1 `UpdateStatus`.
...
**`Create` returns a new Draft (versionNo 1, versionStatus 1).** Set its markup via `UpdateMarkup` (§5), then **publish** it Draft→Ready→Live with `UpdateStatus` (§5.1) — a created-but-unpublished form stays a Draft that end-users can't resolve. Then clear the frontend cache (§11 / [verification.md](verification.md)).
...
## 11. Round-trip verify (post-push) — version-aware (0.43)

Step 8 of the skill. On 0.43, verify the **lifecycle landed**, not just markup ([version-lifecycle.md §Version-aware verification](version-lifecycle.md)):

1. **Latest is Live at the new versionNo** — `GetByName` (no `version` param) → `versionStatus === 3`, `versionNo === <previous + 1>`.
2. **Prior Live is Retired** — `Get?id=<old Live id>` → `versionStatus === 5`.
3. **Diff against the PUBLISHED version**, not the pre-edit id (which may now return the retired version):

```bash
# fetch the NEW Live version's markup by name (latest), not the old $FORM_ID
curl -s -G "$BASE_URL/api/services/Shesha/FormConfiguration/GetByName" \
