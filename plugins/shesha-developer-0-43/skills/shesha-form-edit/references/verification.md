...
> **0.43 is version-aware.** After publishing, confirm the lifecycle landed: `GetByName` (latest) returns `versionStatus === 3` at `versionNo === previous+1`, and the prior Live (`Get?id=<old id>`) is now `versionStatus === 5` (Retired). **Diff against the newly-published version, not the pre-edit `$FORM_ID`** — the old id may now resolve to the retired version and mislead you. Full recipe: [version-lifecycle.md §Version-aware verification](version-lifecycle.md).

`UpdateMarkup` (`PUT /api/services/Shesha/FormConfiguration/UpdateMarkup`) is a **void endpoint**: it returns `success: true` with an **EMPTY/null `result`**. That is normal — it is NOT evidence your markup persisted. (`UpdateStatus` likewise returns void `success: true`.)
...
The frontend caches form configurations in **IndexedDB**. After a successful, API-verified push (and especially after publishing a new version), the browser can keep rendering the **stale cached markup** — you will chase ghosts, "fixing" markup that is already correct. **On 0.43 clear more than `form`/`form_lookup`** — a newly-published version can 404 or render stale unless you also clear the `forms`, `entities`, `ref-lists`, and `misc` IndexedDB stores.
...
// 0.43: clear the full set, not just form/form_lookup
