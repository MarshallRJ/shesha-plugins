...
**If yes** → follow Section 5 of [api.md](api.md) to push lifecycle-aware.

> ⚠ **0.43 lifecycle — cleaning creates a new version.** A form configuration is a **versioned
> ConfigurationItem**, not a mutable blob, and cleaning is a real markup change. You must **never** push
> cleaned markup to a Live id. The push (api.md §5) resolves the current version, and on a **Live** form
> it clones to a **Draft**, pushes the cleaned markup to that Draft, then publishes Draft → Ready → Live
> (auto-retiring the old Live) and clears the frontend cache. A Draft/Ready already in flight is reused
> instead of cloned. The authoritative model is
> [../shesha-form-edit/references/version-lifecycle.md](../shesha-form-edit/references/version-lifecycle.md).

- If the form was loaded via the API (Step 2 Option A), `ACCESS_TOKEN` and the resolution anchor (`FORM_ID` / module+name) are already available — use them directly.
- If the form was loaded from a local file (Step 2 Option B), or `ACCESS_TOKEN` is not available, first follow [api.md](api.md) sections 1–2 to resolve the base URL and authenticate, then ask the user for the form's **module** and **name** (§3) so §5 can resolve the version lifecycle. (Push targets the resolved **Draft** id, never a Live id — do not push directly to a hand-entered Live `itemId`.)
