...
7. Before re-testing, clear the FE form cache: forms are cached in **IndexedDB** (on 0.43 clear the `forms`, `entities`, `ref-lists`, and `misc` stores); `indexedDB.deleteDatabase` from inside the app silently blocks — clear from a static page (e.g. `/favicon.ico`), or stale markup keeps rendering. See [../../shesha-form-edit/references/verification.md](../../shesha-form-edit/references/verification.md).
