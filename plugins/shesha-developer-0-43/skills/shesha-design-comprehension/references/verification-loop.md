...
1. **Build + publish** the form via `shesha-form-edit`, which on 0.43 clones the Live form to a Draft, edits it, and publishes Draft→Ready→Live (auto-retiring the prior Live). This is a versioned lifecycle, not a 2-state Draft→Live swap — the authoritative flow is [../shesha-form-edit/references/version-lifecycle.md](../../shesha-form-edit/references/version-lifecycle.md).
2. **Clear the form cache.** The frontend caches form data in IndexedDB — clear the `forms` / `entities` / `ref-lists` / `misc` stores from `/favicon.ico` (not in-app) after every publish, or you measure a ghost of the previous build. Recipe: [../shesha-form-edit/references/verification.md](../../shesha-form-edit/references/verification.md).
...
4. **Re-probe the newly-Live version.** Run the *same* `scripts/layout-probe.js` against the rendered Shesha form → actual `layout.json`. Same instrument as capture = comparable numbers. Verify you are measuring the just-published version (latest Live, `versionNo`+1), not a retired ghost.
5. **Diff actual vs the blueprint `assertions`** — structurally, not by pixels (next section).
6. **Route mismatches back to `shesha-form-edit`** as concrete fixes. Each fix cycle edits the **same in-flight Draft** and re-publishes it (Draft→Ready→Live) — it does **not** spawn a new version per cycle. Then clear cache → re-probe → re-diff until every assertion passes. (`CreateNewVersion` runs at most once per edit session — see [../shesha-form-edit/references/version-lifecycle.md](../../shesha-form-edit/references/version-lifecycle.md).)
...
- **Stale IndexedDB cache** → you measure the previous build. Always clear the `forms`/`entities`/`ref-lists`/`misc` stores from `/favicon.ico` after every publish.
