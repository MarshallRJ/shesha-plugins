...
5. **Verify by measurement.** Re-probe the newly-Live, table→details-navigated Shesha form — on 0.43 `shesha-form-edit` clones the Live form to a Draft and publishes Draft→Ready→Live, so measure the just-published version (`versionNo`+1), not a retired ghost. Diff actual placement against the blueprint's `assertions`; route concrete mismatches back to `shesha-form-edit`. [verification-loop.md](references/verification-loop.md).
