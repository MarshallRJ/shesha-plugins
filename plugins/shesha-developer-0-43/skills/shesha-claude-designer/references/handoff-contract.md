...
- shesha-form-edit returns: form created/edited (module + name + id), the resolved version-lifecycle facts (`versionNo` + `versionStatus` of the version it wrote — a fresh Draft cloned off the prior Live, or a reused in-flight Draft/Ready), the resolved modelType, the published state (whether that version was advanced Draft→Ready→Live), **and a structural-integrity confirmation** — plus enough to run the placement probe (it builds the form `shesha-design-comprehension` will re-measure). The push/publish lifecycle it follows is authoritatively specified in [../../shesha-form-edit/references/version-lifecycle.md](../../shesha-form-edit/references/version-lifecycle.md).

**Designer → shesha-design-system (Step 3 theme + Step 4b style)**
- Parent provides: token set / theme name, the built form (id/JSON/path), version-lifecycle facts, recipe list.
- shesha-design-system returns: styled JSON (style blocks only, structure untouched), app-theme changes, role→colour trace, audit findings. It does NOT push or publish — it hands styled JSON back for the parent to route through `shesha-form-edit`, which owns the Draft→Ready→Live versioned write (see [../../shesha-form-edit/references/version-lifecycle.md](../../shesha-form-edit/references/version-lifecycle.md)).
