...
Per screen, in order:
- **(a) Structure — REQUIRED SUB-SKILL `shesha-developer-0-43:shesha-form-edit`:** pass the screen's `blueprint.md` as the requirements (archetype → seed/blocks, `layout-tree` spans → **flex `container` rows sized via `desktop.dimensions.width`** — never the `columns` component, `bindings` → component + propertyName). It builds native structure, wires CRUD, validates, then writes and publishes via the 0.43 versioned lifecycle — edits land on a Draft (cloned off the current Live, or a reused in-flight Draft/Ready) which it advances Draft→Ready→Live, auto-retiring the prior Live. Lifecycle authority: [../shesha-form-edit/references/version-lifecycle.md](../shesha-form-edit/references/version-lifecycle.md).
...
- **5a.5 — PLACEMENT diff (REQUIRED `shesha-design-comprehension`):** re-probe the built form once its Draft has been published Live (verify against the newly-Live version, not the retired pre-edit one), table→details-navigated; diff measured column membership / row grouping / nesting depth / tab assignment against the blueprint `assertions`; route concrete mismatches back to `shesha-form-edit`. This is the gate that proves the build matches the design's *layout*, not just that it renders — its method lives in the `shesha-developer-0-43:shesha-design-comprehension` verification loop.
